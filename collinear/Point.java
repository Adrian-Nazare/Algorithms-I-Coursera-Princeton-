...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        if (this.y == that.y) { // if the points are the same
            if (this.x == that.x)
                return Double.NEGATIVE_INFINITY;
            return +0.0; // if the slope is horizontal
        }
        if (this.x == that.x) // if the slope is vertical
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < points.length; i++) System.out.println(points[i].toString());
    }
}
